...
def main():
    ...
    print("Running inspect_dataset...")
    inspect_res = inspect_dataset.run(
        InspectDatasetRequest(source_uri=str(DATA_DIR), dataset_id="fwdftirjune262026", modality_override="FTIR"),
        runs_root=RUNS_ROOT,
    )
    dataset, inspection = load_ftir_real(DATA_DIR)

    print("Running propose_analysis_plan...")
    plan_res = propose_analysis_plan.run(
        ProposeAnalysisPlanRequest(dataset_inspection=inspection, task_hint="classification")
    )
    plan = plan_res.payload

    print("Running run_analysis...")
    run_res = run_analysis.run(
        RunAnalysisRequest(dataset=dataset, approved_plan=plan, run_id="antigravity-ftir-purity-run-export"),
        runs_root=RUNS_ROOT,
    )
    analysis_run = run_res.payload

    print("Running validate_results...")
    val_res = validate_results.run(
        ValidateResultsRequest(results=analysis_run.results, dataset=dataset, dataset_inspection=inspection),
        runs_root=RUNS_ROOT,
    )
    validation_summary = val_res.payload

    print("Running select_best_model...")
    sel_res = select_best_model.run(
        SelectBestModelRequest(results=analysis_run.results, validation_summary=validation_summary, task_name=plan.task_name),
        runs_root=RUNS_ROOT,
    )
    selection_rec = sel_res.payload

    print("Running interpret_results...")
    interp_res = interpret_results.run(
        InterpretResultsRequest(results=analysis_run.results, dataset=dataset, validation_summary=validation_summary),
        runs_root=RUNS_ROOT,
    )
    interpretation_summary = interp_res.payload

    print("Running generate_report...")
    rep_res = generate_report.run(
        GenerateReportRequest(analysis_run=analysis_run, validation_summary=validation_summary, interpretation=interpretation_summary),
        runs_root=RUNS_ROOT,
    )
    ...
    for fname, data_obj in files:
        target_f = TARGET_DIR / fname
        dict_d = to_dict_safe(data_obj)
        txt = json.dumps(dict_d, indent=2, default=str)
        target_f.write_text(txt, encoding="utf-8")
        print(f"Wrote {fname} -> {target_f.stat().st_size} bytes")
    ...
    if report_summary.primary_report and report_summary.primary_report.uri:
        ...
        if rp.exists():
            ...
            print("Copied REPORT.md successfully.")
