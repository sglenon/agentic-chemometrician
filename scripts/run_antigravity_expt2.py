"""Execute full MCP chemometrics analysis for FTIR purity dataset with Ground Truth mapping and output to mcp-expts-2/antigravity."""
...
from chemometrics_mcp.core.datasets import load_ftir_real, load_composition_table
...
COMP_TABLE_PATH = ROOT_DIR / "ftir-purity-dataset/ftir_purity_composition_table.txt"
GT_PATH = ROOT_DIR / "ftir-purity-dataset/ftir-dataset"
TARGET_DIR = ROOT_DIR / "mcp-expts-2/antigravity"
RUNS_ROOT = ROOT_DIR / "runs"
RUN_ID = "antigravity-ftir-purity-run-2"

# Ground Truth Masses from ftir-dataset (mg 3fc, mg 8ami, mg S31)
GT_MG = {
    "C": (0.0, 0.0, 54.0),
    "D": (0.0, 6.5, 17.4),
    "E": (0.0, 16.5, 17.4),
...
def main():
    ...
    # Clean test files
    for tmp_file in ["test.txt", "test_write.json", "hello.txt"]:
        tmp_p = TARGET_DIR / tmp_file
        if tmp_p.exists():
            tmp_p.unlink()
    ...
    # Leakage evaluation: Shuffled 5-Fold vs LOGO
    ...
    # Quantitative Regression evaluation for samples in GT_MG
    ...
    # Save structured json artifacts directly into TARGET_DIR
    for fname, data_obj in [
        ...
        try:
            ...
            print(f"Successfully wrote {fname} ({len(json_text)} bytes)")
        except Exception as err:
            print(f"Error writing {fname}: {err}")

    # Copy primary report if available
    ...
    # Copy generated figures from artifacts
    ...
    # ---------------------------------------------------------
    # GENERATE HIGH-RESOLUTION CUSTOM VISUALIZATIONS
    # ---------------------------------------------------------
    plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')

    # Figure 1: Raw Spectra by Class
    ...
    # Figure 2: SG 1st Derivative Spectra
    plt.figure(figsize=(10, 5))
    for i, lab in enumerate(unique_labels):
        indices = [j for j, l in enumerate(labels) if l == lab]
        for idx in indices:
    ...
    # Figure 3: Ground Truth Composition Bar Chart
    plt.figure(figsize=(10, 5))
    ...
    # Figure 4: Replicate Leakage vs LOGO Validation
    ...
    # Figure 5: PLS Regression Parity Plot for Impurity wt%
    ...
    print(f"\nExecution complete. All artifacts and figures saved to {TARGET_DIR}")
