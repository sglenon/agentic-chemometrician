...
from chemometrics_mcp.core.fallback import _MODEL_SPECIFIC_FALLBACKS  # noqa: E402
...
class TestModeling(unittest.TestCase):
    ...
    # 15b. make_cv_splitter accepts seed parameter
    def test_make_cv_splitter_with_seed(self):
        from sklearn.model_selection import StratifiedKFold
        cv = modeling.make_cv_splitter("stratified_kfold_5", seed=99)
        self.assertIsInstance(cv, StratifiedKFold)
        self.assertEqual(cv.random_state, 99)

    # 15c. run_cv_model_multi_seed returns correct number of results
    def test_run_cv_model_multi_seed_returns_correct_count(self):
        results = modeling.run_cv_model_multi_seed(
            self.X, self.y, self.axis, "svm_rbf",
            "binary_classification", "raw", n_seeds=3,
        )
        self.assertEqual(len(results), 3)

    # 15d. run_cv_model_multi_seed all results have same model_name
    def test_run_cv_model_multi_seed_same_model_name(self):
        results = modeling.run_cv_model_multi_seed(
            self.X, self.y, self.axis, "svm_rbf",
            "binary_classification", "snv", n_seeds=4,
        )
        for result, fig_data in results:
            self.assertEqual(result.model_name, "svm_rbf")
            self.assertIn("confusion_matrix", fig_data)

    # 15e. xgboost classification returns metrics and non-empty selected_features
    def test_xgboost_classification(self):
        result, fig_data = modeling.run_cv_model(
            self.X, self.y, self.axis, "xgboost", self.cv,
            "binary_classification", "raw",
        )
        self.assertIn("accuracy", result.metrics)
        self.assertIn("balanced_accuracy", result.metrics)
    ...
    # 15f. xgboost_reg regression returns r2, rmse, mae
    def test_xgboost_reg_regression(self):
        y_reg = RNG.random(N_SAMPLES)
        from sklearn.model_selection import KFold
        cv_reg = KFold(n_splits=5, shuffle=True, random_state=42)
        result, fig_data = modeling.run_cv_model(
            self.X, y_reg, self.axis, "xgboost_reg", cv_reg,
            "regression", "raw",
        )
    ...
    # 15g. fallback table includes xgboost entries
    def test_fallback_table_includes_xgboost(self):
        self.assertIn("xgboost", _MODEL_SPECIFIC_FALLBACKS)
        self.assertEqual(_MODEL_SPECIFIC_FALLBACKS["xgboost"], "random_forest")
        self.assertIn("xgboost_reg", _MODEL_SPECIFIC_FALLBACKS)
        self.assertEqual(_MODEL_SPECIFIC_FALLBACKS["xgboost_reg"], "plsr")
...
class TestRunAnalysisTool(unittest.TestCase):
    ...
    # 22. Multiple preprocessing candidates produce results for each
    def test_multiple_preprocessing_candidates(self):
        from chemometrics_contracts import RunAnalysisRequest
        dataset = _make_spectral_dataset(include_labels=True)
        plan = AnalysisPlan(
            task_name="binary_classification",
        ...
        preprocessing_values = [r.preprocessing for r in response.payload.results]
        self.assertNotEqual(preprocessing_values[0], preprocessing_values[1])

    # 23. preprocessing_comparison.json artifact is created
    def test_preprocessing_comparison_artifact_created(self):
        from chemometrics_contracts import RunAnalysisRequest
        dataset = _make_spectral_dataset(include_labels=True)
        plan = AnalysisPlan(
            task_name="binary_classification",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for a in response.artifacts:
                if a.kind == "preprocessing_comparison":
                    comparison_artifact = a
                    break
            self.assertIsNotNone(comparison_artifact, "No preprocessing_comparison artifact found")
            comparison_path = Path(comparison_artifact.uri)
            self.assertTrue(comparison_path.exists(), f"preprocessing_comparison.json not found at {comparison_path}")

    # 24. Single preprocessing candidate still works (backward compatible)
    def test_single_preprocessing_backward_compatible(self):
        from chemometrics_contracts import RunAnalysisRequest
        dataset = _make_spectral_dataset(include_labels=True)
        plan = AnalysisPlan(
            task_name="binary_classification",
